...
import datetime
from googlenewsdecoder import new_decod_url
...
def fetch_google_search_news():
    ...
    try:
        ...
        for item in items[:15]: # İlk 15 haberi alıyoruz
            ...
            # 1. Google'ın gizli linkini GERÇEK site linkine çözüyoruz:
            try:
                decoded_res = new_decod_url(google_link)
                if decoded_res and decoded_res.get("status"):
                    real_link = decoded_res.get("decoded_url")
                else:
                    real_link = google_link
            except Exception:
                real_link = google_link

            # 2. Kaynak ve Başlık Ayıklama
            ...
            # 3. Gerçek link geldiyse sizin iframe alanınıza, aksi halde doğrudan haber adresine yönlendir
