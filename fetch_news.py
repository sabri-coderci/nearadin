        ...
        <div class="card">
            ...
            <div class="table-responsive">
                <table class="freq-table">
                    ...
                    <tbody>
                        <tr>
                            <td><strong>TRT 1, TRT Haber, TRT Spor</strong></td>
                        ...
                        <tr>
                            <td><strong>ATV, A Haber, A Spor, A2</strong></td>
                            <td><span class="badge-hd">HD</span></td>
                            <td>12053</td>
                            <td>27500</td>
                            <td>Yatay (H)</td>
                        ...
                        <tr>
                            <td><strong>Show TV, Habertürk</strong></td>
                            <td><span class="badge-hd">HD</span></td>
                            <td>12209</td>
                            <td>10000</td>
                            <td>Yatay (H)</td>
                        ...
                        <tr>
                            <td><strong>Star TV, NTV</strong></td>
                            <td><span class="badge-hd">HD</span></td>
                            <td>12015</td>
                            <td>27500</td>
                            <td>Yatay (H)</td>
...
<head>
    ...
    <style>
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        .results-header {{ font-size: 15px; font-weight: 600; border-bottom: 2px solid #e4e6eb; padding-bottom: 8px; margin-bottom: 15px; }}
        .status-message {{ text-align: center; color: #65676b; padding: 20px 0; font-style: italic; }}
    </style>
</head>
<body>
    {admatic_code}
...
<head>
    ...
    <style>
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        .city-select-box select {{ width: 100%; max-width: 300px; padding: 10px; border-radius: 6px; border: 1px solid #ccd0d5; font-size: 14px; margin-bottom: 20px; }}
        ...
        <div class="card">
            ...
            <div class="city-select-box">
                <select id="citySelect">
                    <option value="34|41.0082|28.9784">34 - İstanbul</option>
...
<head>
    ...
    <style>
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        .film-kart-ozel {{ display: flex; align-items: center; padding: 10px; background: #fff; border: 1px solid #e4e6eb; border-radius: 8px; margin-bottom: 10px; cursor: pointer; }}
    </style>
</head>
<body>
    {admatic_code}
    ...
    <div class="film-container">
        <div style="background:#121212; color:#fff; padding:15px; border-radius:8px; margin-bottom:20px; text-align:center;">
            <h2>🎬 HD Film İzle Portalına Hoş Geldiniz</h2>
        </div>
        {whos_amung_us_code}
    </div>
    {footer_html}
...
def fetch_and_generate():
   ...
   <div class="ad-container">
    ...
    # Statik sayfaları dinamik olarak üret
    ...
    try:
        ...
        # Haber Detay Sayfalarını ve Kart HTML'lerini Üret
...
<body>
    ...
    {footer_html}
            ...
            # Anasayfa için haber kartı oluşturma
            news_cards_html += f'''
            <article style="background: #fff; border: 1px solid #e4e6eb; border-radius: 8px; padding: 16px; margin-bottom: 15px;">
                <div style="font-size: 12px; color: #65676b; margin-bottom: 8px; display: flex; gap: 8px; align-items: center;">
                ...
                <h2 style="font-size: 17px; margin-bottom: 8px; line-height: 1.3;">
                    <a href="{news['internal_link']}" style="color: #0056b3; text-decoration: none;">{news['title']}</a>
                </h2>
                <p style="font-size: 14px; color: #4b4f56; margin-bottom: 10px; line-height: 1.5;">{news['desc'][:160]}...</p>
                <a href="{news['internal_link']}" style="font-size: 13px; font-weight: bold; color: #1877f2; text-decoration: none;">Haberi Oku →</a>
            </article>
            '''

        # 1. Günlük Arşiv Sayfalarını Üret
        for date_folder, data in daily_news_grouped.items():
            daily_cards = ""
            for item in data["news_items"]:
                daily_cards += f'''
                <article style="background: #fff; border: 1px solid #e4e6eb; border-radius: 8px; padding: 14px; margin-bottom: 12px;">
                    <span style="font-size: 11px; color: #65676b;">{item['time']} - {item['source']}</span>
                    <h2 style="font-size: 15px; margin: 4px 0;"><a href="{item['internal_link']}" style="color: #0056b3; text-decoration: none;">{item['title']}</a></h2>
                </article>'''
...
<head>
    <meta charset="UTF-8">
    <title>{data['date_str']} Tarihli Haberler - nearadin.net</title>
    <link rel="canonical" href="https://nearadin.net/haber/{date_folder}/" />
    <style>
        body {{ font-family: sans-serif; background: #f0f2f5; color: #1c1e21; margin: 0; }}
        .container {{ max-width: 680px; margin: 20px auto; padding: 0 12px; }}
        .card {{ background: white; padding: 20px; border-radius: 8px; border: 1px solid #e4e6eb; }}
    </style>
</head>
<body>
    {header_html}
    <div class="container">
        <div class="card">
            <h1 style="font-size: 20px; margin-bottom: 15px; color: #0056b3;">📅 {data['date_str']} Arşivi</h1>
            {daily_cards}
        </div>
    </div>
    {footer_html}
        ...
        # 2. Anasayfa (index.html) Oluşturma
...
<head>
    ...
    <meta name="description" content="Türkiye ve dünyadan en güncel son dakika haberleri, canlı maç sonuçları, Türksat frekansları ve anlık gelişmeler nearadin.net'te!" />
...
<body>
    ...
    <div class="container">
        <div class="card">
            ...
            {news_cards_html if news_cards_html else '<p>Henüz haber bulunamadı.</p>'}
        </div>
        {whos_amung_us_code}
    </div>
    ...
    {footer_html}
        ...
        print(f"✅ Başarılı: Toplam {len(news_list)} haber işlendi, statik sayfalar ve sitemap.xml güncellendi.")

        # En son haberi X (Twitter) üzerinde otomatik paylaş
        if news_list:
